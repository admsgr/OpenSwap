...
/// @dev Version for OperaBasePluginV1
library AdaptiveFee {
  uint16 internal constant INITIAL_MIN_FEE = 0.01e4; // 0.01%

  /// @notice Returns default initial fee configuration
  function initialFeeConfiguration() internal pure returns (OperaFeeConfiguration memory) {
    return
      OperaFeeConfiguration({
  ...
  /// gammas must be > 0
  function validateFeeConfiguration(OperaFeeConfiguration memory _config) internal pure {
  ...
  function getFee(uint88 volatility, OperaFeeConfigurationU144 config) internal pure returns (uint16 fee) {
    unchecked {
