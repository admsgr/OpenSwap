...
import { OperaOracleV1TWAP, MockPool, MockTimeDSFactory, TestERC20 } from '../typechain';
import { tokensFixture } from './shared/externalFixtures';
import { ZERO_ADDRESS } from './shared/fixtures';

describe('OperaOracleV1TWAP', () => {
  let tokens: TestERC20[];
  let operaOracleV1TWAP: OperaOracleV1TWAP;
  let mockPluginFactory: MockTimeDSFactory;

  const operaOracleV1TWAPFixture = async () => {
    ...
    const operaOracleV1TWAPFactory = await ethers.getContractFactory('OperaOracleV1TWAP');
    const _operaOracleV1TWAP = await operaOracleV1TWAPFactory.deploy(_mockPluginFactory);

    return {
      tokens: tokens as TestERC20[],
      operaOracleV1TWAP: _operaOracleV1TWAP as any as OperaOracleV1TWAP,
      mockPluginFactory: _mockPluginFactory as any as MockTimeDSFactory,
    };
  };

  beforeEach('deploy fixture', async () => {
    const fixtures = await loadFixture(operaOracleV1TWAPFixture);
    tokens = fixtures.tokens;
    operaOracleV1TWAP = fixtures.operaOracleV1TWAP;
    mockPluginFactory = fixtures.mockPluginFactory;
  });

  it('has correct pluginFactory', async () => {
    expect(await operaOracleV1TWAP.pluginFactory()).to.be.eq(await mockPluginFactory.getAddress());
  });
  ...
  describe('#getAverageTick', () => {
    ...
    it('reverts if oracle not exist', async () => {
      await expect(operaOracleV1TWAP.getAverageTick(ZERO_ADDRESS, 0)).to.be.revertedWith('Oracle does not exist');
    });
    ...
    describe('plugin connected', async () => {
      it('correct output when tick is 0', async () => {
        ...
        const [oracleLibraryTick, isConnected] = await operaOracleV1TWAP.getAverageTick(mockPool, period);
  ...
  describe('#getQuoteAtTick', () => {
    it('token0: returns correct value when at min tick | 0 < sqrtRatioX96 <= type(uint128).max', async () => {
      const quoteAmount = await operaOracleV1TWAP.getQuoteAtTick(-887272n, 2n ** 128n - 1n, tokens[0], tokens[1]);
  ...
  describe('#latestTimestamp', () => {
    it('returns correct value', async () => {
      ...
      const latestTimestamp = await operaOracleV1TWAP.latestTimestamp(mockVolatilityOracle);
  ...
  describe('#oldestTimestamp', () => {
    it('returns correct value without overflow', async () => {
      ...
      const oldestTimestamp = await operaOracleV1TWAP.oldestTimestamp(mockVolatilityOracle);
  ...
  describe('#latestIndex', () => {
    it('returns correct value', async () => {
      ...
      const latestIndex = await operaOracleV1TWAP.latestIndex(mockVolatilityOracle);
  ...
  describe('#isOracleConnected', () => {
    it('returns correct value', async () => {
      ...
      expect(await operaOracleV1TWAP.isOracleConnected(mockPool)).to.be.false;

      await mockPool.setPlugin(mockVolatilityOracle);
      expect(await operaOracleV1TWAP.isOracleConnected(mockPool)).to.be.false;

      await mockPool.setPluginConfig(1);
      expect(await operaOracleV1TWAP.isOracleConnected(mockPool)).to.be.true;
    });
  });
  ...
  describe('#oldestIndex', () => {
    it('returns correct value without overflow', async () => {
      ...
      const oldestIndex = await operaOracleV1TWAP.oldestIndex(mockVolatilityOracle);
    ...
    it('returns correct value with overflow', async () => {
      ...
      const oldestTimestamp = await operaOracleV1TWAP.oldestIndex(mockVolatilityOracle);
