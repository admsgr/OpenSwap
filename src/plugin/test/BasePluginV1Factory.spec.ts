...
import { BasePluginV1Factory, OperaBasePluginV1, MockFactory } from '../typechain';

describe('BasePluginV1Factory', () => {
  let wallet: Wallet, other: Wallet;

  let pluginFactory: BasePluginV1Factory;
  let mockOperaFactory: MockFactory;
  ...
  beforeEach('deploy test volatilityOracle', async () => {
    ({ pluginFactory, mockFactory: mockOperaFactory } = await loadFixture(pluginFactoryFixture));
  });
  ...
  describe('#Create plugin', () => {
    ...
    it('factory can create plugin', async () => {
      ...
      const pluginMock = (await ethers.getContractFactory('OperaBasePluginV1')).attach(pluginAddress) as any as OperaBasePluginV1;
  ...
  describe('#CreatePluginForExistingPool', () => {
    ...
    it('can create for existing pool', async () => {
      await mockOperaFactory.stubPool(wallet.address, other.address, other.address);
