...
import { MockTimeOperaPool } from '../../core/typechain';
import { MockTimeOperaBasePluginV1, MockTimeDSFactory, MockTimeVirtualPool } from '../typechain';
import { expect } from './shared/expect';

import { operaPoolDeployerMockFixture } from './shared/externalFixtures';
...
describe('OperaPool gas tests [ @skip-on-coverage ]', () => {
  ...
  async function gasTestFixture() {
    const fix = await operaPoolDeployerMockFixture();
    ...
    const mockDSOperatorFactory = await ethers.getContractFactory('MockTimeOperaBasePluginV1');
    const plugin = mockDSOperatorFactory.attach(pluginAddress) as any as MockTimeOperaBasePluginV1;
  ...
  let plugin: MockTimeOperaBasePluginV1;
