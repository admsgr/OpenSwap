...
import { OperaEternalFarming, TestERC20 } from '../typechain';
import { ethers } from 'hardhat';
import { blockTimestamp, BNe18, expect, FeeAmount, getMaxTick, getMinTick, TICK_SPACINGS, operaFixture, days, bnSum, mintPosition } from './shared';
...
describe('OperaFarming', () => {
  ...
  describe('minimal position width', async () => {
    ...
    const scenario: () => Promise<TestSubject> = async () => {
      const context = await operaFixture();
    ...
    it('too wide range cannot be used as minimal allowed', async () => {
      ...
      await expect(
        (context.eternalFarming as OperaEternalFarming).connect(incentiveCreator).createEternalFarming(
          {
            pool: context.pool01,
      ...
      ).to.be.revertedWithCustomError(context.eternalFarming as OperaEternalFarming, 'minimalPositionWidthTooWide');
    ...
    it('max range can be used as minimal allowed', async () => {
      ...
      ).to.be.revertedWithCustomError(context.eternalFarming as OperaEternalFarming, 'positionIsTooNarrow');
