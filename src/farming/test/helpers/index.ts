...
import { TestERC20, INonfungiblePositionManager, OperaEternalFarming, IOperaPool, TestIncentiveId, FarmingCenter } from '../../typechain';
...
import { ISwapRouter } from '@openswap/opera-insider-periphery/typechain';
...
export class HelperCommands {
  ...
  createIncentiveFlow: HelperTypes.CreateIncentive.Command = async (params) => {
    ...
    if (!pluginAddres) {
      const pool = (await ethers.getContractAt('IOperaPool', params.poolAddress)) as any as IOperaPool;
      pluginAddres = await pool.connect(incentiveCreator).plugin();
    }

    txResult = await (this.eternalFarming as OperaEternalFarming).connect(incentiveCreator).createEternalFarming(
      {
