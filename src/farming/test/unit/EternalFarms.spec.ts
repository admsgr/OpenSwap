...
import { TestERC20, OperaEternalFarming, EternalVirtualPool, TestERC20Reentrant } from '../../typechain';
import { mintPosition, OperaFixtureType, operaFixture } from '../shared/fixtures';
import {
  expect,
  getMaxTick,
  getMinTick,
...
describe('unit/EternalFarms', () => {
  ...
  const detachIncentiveIndirectly = async (localNonce: any) => {
    await context.pluginFactory.setFarmingAddress(actors.operaRootUser().address);

    const incentiveAddress = await context.pluginObj.connect(actors.operaRootUser()).incentive();
    ...
    await context.pluginObj.connect(actors.operaRootUser()).setIncentive(ZERO_ADDRESS);

    const tick = (await context.poolObj.connect(actors.operaRootUser()).globalState()).tick;

    await helpers.moveTickTo({ direction: 'down', desiredValue: Number(tick) - 130, trader: actors.farmingDeployer() });

    await context.pluginObj.connect(actors.operaRootUser()).setIncentive(incentiveAddress);
  ...
  describe('#createEternalFarming', () => {
    ...
    it('cannot create farming without rewards', async () => {
      ...
      await expect(helpers.createIncentiveFlow(incentiveArgs)).to.be.revertedWithCustomError(
        context.eternalFarming as OperaEternalFarming,
        'zeroRewardAmount'
      );
    });
  ...
  describe('#enterFarming', () => {
    ...
    describe('fails when', () => {
      ...
      it('has 0 liquidity in the position', async () => {
        ...
        await expect(subject(tokenId2, lpUser0)).to.be.revertedWithCustomError(context.eternalFarming as OperaEternalFarming, 'zeroLiquidity');
      });
      ...
      it('token id is for a different pool than the incentive', async () => {
        ...
        ).to.be.revertedWithCustomError(context.eternalFarming as OperaEternalFarming, 'invalidPool');
      });
      ...
      it('incentive key does not exist', async () => {
        ...
        ).to.be.revertedWithCustomError(context.eternalFarming as OperaEternalFarming, 'incentiveNotExist');
      });
    });
  });
