...
import { IOperaPool, TestERC20 } from '../../typechain';
...
export const getSlot0 = async (pool: IOperaPool) => {
  return await pool.globalState();
};

// This is currently lpUser0 but can be called from anybody.
export const getCurrentTick = async (pool: IOperaPool): Promise<number> => Number((await getSlot0(pool)).tick);
