...
import OperaPool from '@openswap/opera-insider-core/artifacts/contracts/OperaPool.sol/OperaPool.json';
import OperaFactoryJson from '@openswap/opera-insider-core/artifacts/contracts/OperaFactory.sol/OperaFactory.json';
import OperaPoolDeployerJson from '@openswap/opera-insider-core/artifacts/contracts/OperaPoolDeployer.sol/OperaPoolDeployer.json';
import NFTDescriptorJson from '@openswap/opera-insider-periphery/artifacts/contracts/libraries/NFTDescriptor.sol/NFTDescriptor.json';
import NonfungiblePositionManagerJson from '@openswap/opera-insider-periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json';
import NonfungibleTokenPositionDescriptor from '@openswap/opera-insider-periphery/artifacts/contracts/NonfungibleTokenPositionDescriptor.sol/NonfungibleTokenPositionDescriptor.json';
import SwapRouter from '@openswap/opera-insider-periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json';
import WNativeToken from './external/WNativeToken.json';
import { linkLibraries } from './linkLibraries';
import { ISwapRouter, IWNativeToken, NFTDescriptor } from '@openswap/opera-insider-periphery/typechain';
import {
...
} from '@openswap/opera-insider-base-plugin/artifacts/contracts/OperaBasePluginV1.sol/OperaBasePluginV1.json';
import {
  OperaEternalFarming,
  TestERC20,
...
import { IBasePluginV1Factory, IOperaBasePluginV1 } from '@openswap/opera-insider-base-plugin/typechain';
...
const v3CoreFactoryFixture: () => Promise<[IOperaFactory, IOperaPoolDeployer, IBasePluginV1Factory, Signer]> = async () => {
  ...
  const v3FactoryFactory = await ethers.getContractFactory(OperaFactoryJson.abi, OperaFactoryJson.bytecode);
  const _factory = (await v3FactoryFactory.deploy(poolDeployerAddress)) as any as IOperaFactory;

  const poolDeployerFactory = await ethers.getContractFactory(OperaPoolDeployerJson.abi, OperaPoolDeployerJson.bytecode);
  const _deployer = (await poolDeployerFactory.deploy(_factory)) as any as IOperaPoolDeployer;
...
export const operaFactoryFixture: () => Promise<OperaFactoryFixture> = async () => {
...
export type OperaFixtureType = {
  ...
  factory: IOperaFactory;
  poolObj: IOperaPool;
  pluginObj: IOperaBasePluginV1;
  pluginFactory: IBasePluginV1Factory;
  router: ISwapRouter;
  eternalFarming: OperaEternalFarming;
...
export const operaFixture: () => Promise<OperaFixtureType> = async () => {
  const { tokens, nft, factory, deployer, router, pluginFactory, ownerSigner } = await operaFactoryFixture();
  ...
  const eternalFarmingFactory = await ethers.getContractFactory('OperaEternalFarming', signer);
  const eternalFarming = (await eternalFarmingFactory.deploy(deployer, nft)) as any as OperaEternalFarming;
  ...
  const poolObj = poolFactory.attach(pool01) as any as IOperaPool;

  const pluginContractFactory = new ethers.ContractFactory(PLUGIN_ABI, PLUGIN_BYTECODE, signer);

  const pluginObj = pluginContractFactory.attach(await poolObj.connect(signer).plugin()) as any as IOperaBasePluginV1;
...
export const poolFactory = new ethers.ContractFactory(OperaPool.abi, OperaPool.bytecode);
