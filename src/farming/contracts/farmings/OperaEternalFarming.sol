...
import '@openswap/opera-insider-core/contracts/libraries/LowGasSafeMath.sol';

import '@openswap/opera-insider-periphery/contracts/libraries/TransferHelper.sol';

import '@openswap/opera-insider-base-plugin/contracts/interfaces/plugins/IFarmingPlugin.sol';

import '../interfaces/IOperaEternalFarming.sol';
import '../interfaces/IOperaEternalVirtualPool.sol';
...
/// @title Opera Insider 1.1  eternal (v2-like) farming
/// @notice Manages rewards and virtual pools
contract OperaEternalFarming is IOperaEternalFarming {
  ...
  /// @inheritdoc IOperaEternalFarming
  ...
  IOperaPoolDeployer private immutable deployer;
  IOperaFactory private immutable factory;
  ...
  constructor(IOperaPoolDeployer _deployer, INonfungiblePositionManager _nonfungiblePositionManager) {
    (deployer, nonfungiblePositionManager) = (_deployer, _nonfungiblePositionManager);
    factory = IOperaFactory(_nonfungiblePositionManager.factory());
  }
  ...
  function createEternalFarming(
    ...
    _addRewards(IOperaEternalVirtualPool(virtualPool), params.reward, params.bonusReward, incentiveId);
    _setRewardRates(IOperaEternalVirtualPool(virtualPool), params.rewardRate, params.bonusRewardRate, incentiveId);
  }
  ...
  function deactivateIncentive(IncentiveKey memory key) external override onlyIncentiveMaker {
    ...
    IOperaEternalVirtualPool virtualPool = IOperaEternalVirtualPool(incentive.virtualPoolAddress);
  ...
  function enterFarming(IncentiveKey memory key, uint256 tokenId) external override onlyFarmingCenter {
    ...
    IOperaEternalVirtualPool virtualPool = IOperaEternalVirtualPool(virtualPoolAddress);
  ...
  /// @dev Does not check if the incentive is indeed currently connected to the Opera pool or not
  function _isIncentiveDeactivated(Incentive storage incentive) private view returns (bool) {
    ...
    if (!_deactivated) {
      _deactivated = IOperaEternalVirtualPool(virtualPoolAddress).deactivated(); // if incentive was deactivated automatically
    }
    return _deactivated;
  }

  function _getInnerRewardsGrowth(IOperaEternalVirtualPool virtualPool, int24 tickLower, int24 tickUpper) private view returns (uint256, uint256) {
    return virtualPool.getInnerRewardsGrowth(tickLower, tickUpper);
  }

  function _getNewRewardsForFarm(
    IOperaEternalVirtualPool virtualPool,
    Farm memory farm
  ...
  function _distributeRewards(IOperaEternalVirtualPool virtualPool) private {
    virtualPool.distributeRewards();
  }

  function _addRewards(IOperaEternalVirtualPool virtualPool, uint128 amount0, uint128 amount1, bytes32 incentiveId) private {
    virtualPool.addRewards(amount0, amount1);
    emit RewardsAdded(amount0, amount1, incentiveId);
  }

  function _setRewardRates(IOperaEternalVirtualPool virtualPool, uint128 rate0, uint128 rate1, bytes32 incentiveId) private {
  ...
  function _getTickInPoolAndCheckLock(IOperaPool pool) internal view returns (int24 tick) {
    bool poolUnlocked;
  ...
  function _updatePositionInVirtualPool(address virtualPool, int24 tickLower, int24 tickUpper, int128 liquidityDelta, int24 currentTick) internal {
    IOperaEternalVirtualPool(virtualPool).applyLiquidityDeltaToPosition(tickLower, tickUpper, liquidityDelta, currentTick);
  }
}
