...
import '@openswap/opera-insider-periphery/contracts/interfaces/IMulticall.sol';
...
interface IFarmingCenter is IMulticall {
  ...
  function eternalFarming() external view returns (IOperaEternalFarming);

  /// @notice The Opera poolDeployer contract
  function operaPoolDeployer() external view returns (address);
  ...
  function incentiveKeys(
    bytes32 incentiveId
  ) external view returns (IERC20Minimal rewardToken, IERC20Minimal bonusRewardToken, IOperaPool pool, uint256 nonce);

  /// @notice Used to connect incentive to compatible OperaPool plugin
  ...
  /// @param plugin The Opera farming plugin
  function connectVirtualPoolToPlugin(address virtualPool, IFarmingPlugin plugin) external;

  /// @notice Used to disconnect incentive from compatible OperaPool plugin
  /// @dev only farming can do it.
