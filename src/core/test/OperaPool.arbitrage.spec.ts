...
import { MockTimeOperaPool, TickMathTest, OperaPoolSwapTest } from '../typechain';
...
describe('OperaPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async () => {
            ...
            const testerFactory = await ethers.getContractFactory('OperaPoolSwapTest');
            const tester = (await testerFactory.deploy()) as any as OperaPoolSwapTest;
