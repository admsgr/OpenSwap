...
describe('OperaPool', () => {
  ...
  let pool: MockTimeOperaPool;

  let swapTarget: TestOperaCallee;
  ...
  it('isUnlocked', async () => {
    expect(await pool.isUnlocked()).to.be.true; // false checked inside of TestOperaReentrantCallee.sol
  });
  ...
  it('safelyGetStateOfAMM', async () => {
    ...
    // revert case checked inside of TestOperaReentrantCallee.sol
  });
    ...
    describe('after initialization', () => {
      ...
      describe('failure cases', () => {
        describe('underpayment', () => {
          let payer: TestOperaSwapPay;

          beforeEach(async () => {
            const factory = await ethers.getContractFactory('TestOperaSwapPay');
            payer = (await factory.deploy()) as any as TestOperaSwapPay;
      ...
      describe('success cases', () => {
        ...
        it('refund if overpayment', async () => {
          const factory = await ethers.getContractFactory('TestOperaSwapPay');
          let payer = (await factory.deploy()) as any as TestOperaSwapPay;
          await token0.approve(payer, 2n ** 256n - 1n);
          await token1.approve(payer, 2n ** 256n - 1n);
  ...
  async function initializeAtZeroTick(pool: MockTimeOperaPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1));
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestOperaReentrantCallee')
      ).deploy()) as any as TestOperaReentrantCallee;
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestOperaSwapPay;
    let poolAddress: string;
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestOperaSwapPay');
      underpay = (await underpayFactory.deploy()) as any as TestOperaSwapPay;
