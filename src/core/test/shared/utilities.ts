...
import { TestOperaCallee, TestOperaRouter, MockTimeOperaPool, TestERC20 } from '../../typechain';
...
export function getCreate2CustomAddress(
  poolDeployerAddress: string,
  customDeployerAddress: string,
  [tokenA, tokenB]: [string, string],
  bytecode: string
): string {
  const [token0, token1] = tokenA.toLowerCase() < tokenB.toLowerCase() ? [tokenA, tokenB] : [tokenB, tokenA];
  const constructorArgumentsEncoded = AbiCoder.defaultAbiCoder().encode(
    ...
    [customDeployerAddress, token0, token1]
  );
  const create2Inputs = [
    '0xff',
    poolDeployerAddress,
...
export function getPositionKey(
  ...
  bottomTick: number,
  topTick: number,
  pool: MockTimeOperaPool
): Promise<string> {
  return pool.getKeyForPosition(address, bottomTick, topTick);
}

export function getLimitPositionKey(address: string, tick: number, pool: MockTimeOperaPool): Promise<string> {
...
export function createMultiPoolFunctions({
  ...
  inputToken: TestERC20;
  swapTarget: TestOperaRouter;
  poolInput: MockTimeOperaPool;
  poolOutput: MockTimeOperaPool;
}): MultiPoolFunctions {
