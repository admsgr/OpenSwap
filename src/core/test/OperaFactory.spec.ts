...
import { OperaFactory, OperaPoolDeployer, IOperaFactory, MockDefaultPluginFactory, TestOperaReentrantCallee } from '../typechain';
...
describe('OperaFactory', () => {
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('OperaPool')).bytecode;
  });
  ...
  async function createAndCheckPool(tokens: [string, string]) {
    ...
    const poolContractFactory = await ethers.getContractFactory('OperaPool');
  ...
  describe('#createCustomPool', () => {
    ...
    async function createAndCheckCustomPool(_factory: IOperaFactory, tokens: [string, string], data: string = '0x') {
