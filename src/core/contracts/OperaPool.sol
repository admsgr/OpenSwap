...
import './interfaces/plugin/IOperaPlugin.sol';
import './interfaces/IOperaFactory.sol';

/// @title Opera concentrated liquidity pool
/// @notice This contract is responsible for liquidity positions, swaps and flashloans
/// @dev Version: Opera Insider 1.1
contract OperaPool is OperaPoolBase, TickStructure, ReentrancyGuard, Positions, SwapCalculation, ReservesManager {
  ...
  function initialize(uint160 initialPrice) external override {
    ...
    if (plugin != address(0)) {
      IOperaPlugin(plugin).beforeInitialize(msg.sender, initialPrice).shouldReturn(IOperaPlugin.beforeInitialize.selector);
    }
    ...
    if (globalState.pluginConfig.hasFlag(Plugins.AFTER_INIT_FLAG)) {
      IOperaPlugin(plugin).afterInitialize(msg.sender, initialPrice, tick).shouldReturn(IOperaPlugin.afterInitialize.selector);
    }
  }
  ...
  function mint(
    ...
    _mintCallback(amount0, amount1, data); // IOperaMintCallback.operaMintCallback to msg.sender
  ...
  function _beforeModifyPos(address owner, int24 bottomTick, int24 topTick, int128 liquidityDelta, bytes calldata data) internal {
    if (globalState.pluginConfig.hasFlag(Plugins.BEFORE_POSITION_MODIFY_FLAG)) {
      IOperaPlugin(plugin).beforeModifyPosition(msg.sender, owner, bottomTick, topTick, liquidityDelta, data).shouldReturn(
        IOperaPlugin.beforeModifyPosition.selector
      );
    }
  }
  ...
  function _afterModifyPos(address owner, int24 bTick, int24 tTick, int128 deltaL, uint256 amount0, uint256 amount1, bytes calldata data) internal {
    if (globalState.pluginConfig.hasFlag(Plugins.AFTER_POSITION_MODIFY_FLAG)) {
      IOperaPlugin(plugin).afterModifyPosition(msg.sender, owner, bTick, tTick, deltaL, amount0, amount1, data).shouldReturn(
        IOperaPlugin.afterModifyPosition.selector
      );
    }
  }
  ...
  function _beforeSwap(address recipient, bool zto, int256 amount, uint160 limitPrice, bool payInAdvance, bytes calldata data) internal {
    if (globalState.pluginConfig.hasFlag(Plugins.BEFORE_SWAP_FLAG)) {
      IOperaPlugin(plugin).beforeSwap(msg.sender, recipient, zto, amount, limitPrice, payInAdvance, data).shouldReturn(
        IOperaPlugin.beforeSwap.selector
      );
    }
  }
  ...
  function _afterSwap(address recipient, bool zto, int256 amount, uint160 limitPrice, int256 amount0, int256 amount1, bytes calldata data) internal {
    if (globalState.pluginConfig.hasFlag(Plugins.AFTER_SWAP_FLAG)) {
      IOperaPlugin(plugin).afterSwap(msg.sender, recipient, zto, amount, limitPrice, amount0, amount1, data).shouldReturn(
        IOperaPlugin.afterSwap.selector
      );
    }
  }
  ...
  function flash(address recipient, uint256 amount0, uint256 amount1, bytes calldata data) external override {
    if (globalState.pluginConfig.hasFlag(Plugins.BEFORE_FLASH_FLAG)) {
      IOperaPlugin(plugin).beforeFlash(msg.sender, recipient, amount0, amount1, data).shouldReturn(IOperaPlugin.beforeFlash.selector);
    }
    _lock();
    ...
    uint256 paid1;
    {
      ...
      _flashCallback(fee0, fee1, data); // IOperaFlashCallback.operaFlashCallback to msg.sender
    ...
    if (globalState.pluginConfig.hasFlag(Plugins.AFTER_FLASH_FLAG)) {
      IOperaPlugin(plugin).afterFlash(msg.sender, recipient, amount0, amount1, paid0, paid1, data).shouldReturn(IOperaPlugin.afterFlash.selector);
    }
  }

  /// @dev using function to save bytecode
  function _checkIfAdministrator() private view {
    if (!IOperaFactory(factory).hasRoleOrOwner(Constants.POOLS_ADMINISTRATOR_ROLE, msg.sender)) revert notAllowed();
  }
  ...
  /// @inheritdoc IOperaPoolPermissionedActions
  function setFedReserves(uint256 _r0, uint256 _r1) external override onlyUnlocked {
    if (msg.sender != factory) _checkIfAdministrator();
    _setFedReserves(_r0, _r1);
  }
