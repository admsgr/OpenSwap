...
import '../interfaces/pool/IOperaPoolErrors.sol';
...
library TickManagement {
  ...
  function checkTickRangeValidity(int24 bottomTick, int24 topTick) internal pure {
    if (topTick > TickMath.MAX_TICK) revert IOperaPoolErrors.topTickAboveMAX();
    if (topTick <= bottomTick) revert IOperaPoolErrors.topTickLowerOrEqBottomTick();
    if (bottomTick < TickMath.MIN_TICK) revert IOperaPoolErrors.bottomTickLowerThanMIN();
  }
  ...
  function update(
    ...
    if (liquidityTotalAfter > Constants.MAX_LIQUIDITY_PER_TICK) revert IOperaPoolErrors.liquidityOverflow();
  ...
  function removeTick(mapping(int24 => Tick) storage self, int24 tick) internal returns (int24 prevTick, int24 nextTick) {
    ...
    if (tick == TickMath.MIN_TICK || tick == TickMath.MAX_TICK) {
      ...
      if (prevTick == nextTick) revert IOperaPoolErrors.tickIsNotInitialized();
  ...
  function insertTick(mapping(int24 => Tick) storage self, int24 tick, int24 prevTick, int24 nextTick) internal {
    ...
    if (!(prevTick < tick && nextTick > tick)) revert IOperaPoolErrors.tickInvalidLinks();
    (self[tick].prevTick, self[tick].nextTick) = (prevTick, nextTick);
