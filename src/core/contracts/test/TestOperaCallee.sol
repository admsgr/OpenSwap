...
contract TestOperaCallee is IOperaMintCallback, IOperaSwapCallback, IOperaFlashCallback {
  ...
  function swapExact0For1(address pool, uint256 amount0In, address recipient, uint160 limitSqrtPrice) external {
    IOperaPool(pool).swap(recipient, true, int256(amount0In), limitSqrtPrice, abi.encode(msg.sender));
  }

  function swapExact0For1SupportingFee(address pool, uint256 amount0In, address recipient, uint160 limitSqrtPrice) external {
    IOperaPool(pool).swapWithPaymentInAdvance(msg.sender, recipient, true, int256(amount0In), limitSqrtPrice, abi.encode(msg.sender));
  }

  function swap0ForExact1(address pool, uint256 amount1Out, address recipient, uint160 limitSqrtPrice) external {
    unchecked {
      IOperaPool(pool).swap(recipient, true, -amount1Out.toInt256(), limitSqrtPrice, abi.encode(msg.sender));
    }
  }

  function swapExact1For0(address pool, uint256 amount1In, address recipient, uint160 limitSqrtPrice) external {
    IOperaPool(pool).swap(recipient, false, int256(amount1In), limitSqrtPrice, abi.encode(msg.sender));
  }

  function swapExact1For0SupportingFee(address pool, uint256 amount1In, address recipient, uint160 limitSqrtPrice) external {
    IOperaPool(pool).swapWithPaymentInAdvance(msg.sender, recipient, false, int256(amount1In), limitSqrtPrice, abi.encode(msg.sender));
  }

  function swap1ForExact0(address pool, uint256 amount0Out, address recipient, uint160 limitSqrtPrice) external {
    unchecked {
      IOperaPool(pool).swap(recipient, false, -amount0Out.toInt256(), limitSqrtPrice, abi.encode(msg.sender));
    }
  }

  function swapToLowerSqrtPrice(address pool, uint160 price, address recipient) external {
    IOperaPool(pool).swap(recipient, true, type(int256).max, price, abi.encode(msg.sender));
  }

  function swapToHigherSqrtPrice(address pool, uint160 price, address recipient) external {
    IOperaPool(pool).swap(recipient, false, type(int256).max, price, abi.encode(msg.sender));
  }
  ...
  function mint(
    ...
    (amount0Owed, amount1Owed, resultLiquidity) = IOperaPool(pool).mint(msg.sender, recipient, bottomTick, topTick, amount, abi.encode(msg.sender));
  ...
  function operaMintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
    ...
    if (amount0Owed > 0) IERC20Minimal(IOperaPool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
    if (amount1Owed > 0) IERC20Minimal(IOperaPool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
  ...
  function flash(address pool, address recipient, uint256 amount0, uint256 amount1, uint256 pay0, uint256 pay1) external {
    IOperaPool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
  }

  function operaFlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
    emit FlashCallback(fee0, fee1);

    (address sender, uint256 pay0, uint256 pay1) = abi.decode(data, (address, uint256, uint256));

    if (pay0 > 0) IERC20Minimal(IOperaPool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
    if (pay1 > 0) IERC20Minimal(IOperaPool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
  }
