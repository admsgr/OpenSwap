...
import './MockTimeOperaPool.sol';

contract MockTimeOperaPoolDeployer {
  ...
  constructor() {
    mockPoolHash = keccak256(type(MockTimeOperaPool).creationCode);
  }
  ...
  function deployMock(address _factory, address token0, address token1) external returns (address pool) {
    ...
    pool = address(new MockTimeOperaPool{salt: keccak256(abi.encode(token0, token1))}());
    (cache0, cache1) = (bytes32(0), bytes32(0));
    emit PoolDeployed(pool);

    IOperaVaultFactory _vaultFactory = IOperaFactory(_factory).vaultFactory();
    address _vault = _vaultFactory.getVaultForPool(pool);
    IOperaPool(pool).setCommunityVault(_vault);
  }
