...
/// @title Opera reserves management abstract contract
...
abstract contract ReservesManager is OperaPoolBase {
  ...
  /// @param _oneByteReserves should be uint8 max of reserves or not
  function _updateReserves(bool _oneByteReserves) internal returns (uint256 balance0, uint256 balance1) {
    ...
    if (_oneByteReserves == true) {
      _transfer(token0, communityVault, balance0 - type(uint8).max);
      _transfer(token1, communityVault, balance1 - type(uint8).max);
    }
    unchecked {
