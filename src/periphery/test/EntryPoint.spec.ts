...
import { abi as IOperaPoolABI } from '@openswap/opera-insider-core/artifacts/contracts/interfaces/IOperaPool.sol/IOperaPool.json';
...
describe('CustomPoolEntryPoint', () => {
  ...
  let factory: IOperaFactory;
  let nft: MockTimeNonfungiblePositionManager;
  let tokens: [TestERC20, TestERC20, TestERC20];
  let wnative: IWNativeToken;
  let entryPoint: OperaCustomPoolEntryPoint;
  ...
  beforeEach('load fixture', async () => {
    ...
    const entryPointFactory = await ethers.getContractFactory('OperaCustomPoolEntryPoint');
    entryPoint = (await entryPointFactory.deploy(factory)) as any as OperaCustomPoolEntryPoint;
  ...
  describe('create custom pool via entry point', () => {
    ...
    it('pool configuration can be changed by customPoolDeployer contract', async () => {
      ...
      const pool = new ethers.Contract(customPoolAddress, IOperaPoolABI, wallet);
