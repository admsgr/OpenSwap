...
const OperaFactoryComplied = require('@openswap/opera-insider-core/artifacts/contracts/OperaFactory.sol/OperaFactory.json');
...
async function main() {
  ...
  // WNativeTokenAddress
  const WNativeTokenAddress = deploysData.wFTM;
  const signers = await hre.ethers.getSigners();
  const ProxyAdmin = signers[2].address;

  deploysData.wrapped = WNativeTokenAddress;

  const entryPointFactory = await hre.ethers.getContractFactory('OperaCustomPoolEntryPoint');
  ...
  const factory = await hre.ethers.getContractAt(OperaFactoryComplied.abi, deploysData.factory);
  ...
  const OperaInterfaceMulticallFactory = await hre.ethers.getContractFactory('OperaInterfaceMulticall');
  const OperaInterfaceMulticall = await OperaInterfaceMulticallFactory.deploy();

  await OperaInterfaceMulticall.waitForDeployment();

  console.log('OperaInterfaceMulticall deployed to:', OperaInterfaceMulticall.target);
